...
use Aliyun\PDS\SDK\Models\GetAppPublicKeyRequest;
...
use Aliyun\PDS\SDK\Models\GetStoryTaskModel;
use Aliyun\PDS\SDK\Models\GetStoryTaskRequest;
...
class pds
{
     ...
     * @error ServiceUnavailable The request has failed due to a temporary failure of the server.
     *
     * @param GetStoryTaskRequest $request
     * @param RuntimeOptions      $runtime
     *
     * @throws TeaError
     * @throws Exception
     ...
     */
    public function getStoryTaskEx($request, $runtime)
    {
        $request->validate();
        $runtime->validate();
        $_runtime = [
        ...
        while (Tea::allowRetry($_runtime['retry'], $_retryTimes, $_now)) {
            ...
            try {
                ...
                $_request->protocol = Utils::defaultString($this->_protocol, 'https');
                $_request->method   = 'POST';
                $_request->pathname = $this->getPathname($this->_nickname, '/v2/image/get_story_task');
                $_request->headers  = Tea::merge([
                    'user-agent'   => $this->getUserAgent(),
                    'host'         => Utils::defaultString($this->_endpoint, '' . $this->_domainId . '.api.aliyunpds.com'),
                    'content-type' => 'application/json; charset=utf-8',
                ...
                if (Utils::equalNumber($_response->statusCode, 200)) {
                    ...
                    return GetStoryTaskModel::fromMap([
                        'body'    => $respMap,
                        'headers' => $_response->headers,
                    ]);
                }
     ...
     * @return GetStoryTaskModel
     */
    public function getStoryTask($request)
    {
        $runtime = new RuntimeOptions([]);
